...
from recipes import STYLES, QUALITY_GEOMETRY, variant_slug
...
def run(args, out, frozen_root, snapshot):
    ...
    slug = variant_slug(args.style, args.quality)
    scene = build_scene(args.style, args.quality)
    create_proxies()
    ...
    bpy.ops.wm.save_as_mainfile(filepath=str(out / (slug + "-metric-study.blend")))
    glb = export_study_glb(out / (slug + "-metric-study.glb"))
    ...
    for view in (() if args.no_render else ("facade", "interior")):
        ...
        scene.render.filepath = str(out / (slug + "-" + view + "-offline-study.png"))
    ...
    report = {"kind": "c12-original-metric-study", "status": "incomplete-unapproved", "publicationAllowed": False,
              ...
              "studyTheme": STYLES[args.style], "style": args.style, "quality": args.quality,
              "canonicalRegistryReviewThemeUnchanged": row["theme_revue_v3"],
              "themeReason": "Modern1 standard is primary comparison; five exploratory recipes do not constitute final artistic acceptance",
              "geometryRecipe": QUALITY_GEOMETRY[args.quality], "canonicalImagesDelivered": 0,
              "qualityQualification": "offline-geometry-and-export-only; GPU/draw-calls/FPS/runtime not measured",
              ...
              "blendBytes": (out / (slug + "-metric-study.blend")).stat().st_size}
