...
Écarts assumés entre maquettes et v1 (R4, R17) : attribut de liaison « Quantité » ~~désactivé « Bientôt »~~ **livré en v1.1** (dialogue N-N actif, quantité dans « Liés », puces inline dans la fiche — D-47-40) ; onglets « Déléguées » / « Historique », « Relancer », « Tester sur un enregistrement », « Historique des modifications » / « Pièces jointes » omis ; micro « Bientôt » ; déclencheur planifié désactivé.
...
| 2026-09-17 | 4.4 | D-44-56 — Bascule colonne 372 px / tiroir. | `matchMedia('(min-width: 1280px)')` + `DestroyRef`, sans `BreakpointObserver` (h1). | Responsive — **Clos en 4.6T2** (D-46-T02 : `ViewportService`) |
...
| 2026-09-18 | 4.5 | D-45-27 — Revue sécurité ★ : la route lecteur renvoyait le `context` complet de l'instance (`startedBy.email`, `results` = sorties ERP non tronquées, `vars`), données jamais rendues par le tiroir. | Corrigé en 4.5i★ : `StudioWorkflowInstanceDetailBuilder.BuildAsync(…, readerScope: true)` pour `GetRecordWorkflowInstanceQueryHandler` — `startedBy.email` = null, `results` et `vars` = `{}` (clés conservées) ; route de conception intacte et verrouillée par un test symétrique. Résiduel accepté : `Steps[].Result` (sorties tronquées) reste servi au lecteur — à nuller en `readerScope` si une surface strictement minimale est exigée **Levé en 4.6b2** (D-46-05). | Aucun (le tiroir ne lit pas `context`) |
| 2026-09-18 | 4.5 | D-45-28 — Revue ★ : `?page=2147483647` sur `GET workflows` débordait dans `(page - 1) * pageSize` ⇒ `Skip` négatif ⇒ 500 (concepteur authentifié, sans fuite). | Corrigé : `page` borné à `int.MaxValue / MaxPageSize` dans le handler ; +1 test. Les jokers `LIKE` (`%`, `_`) de `search` sont paramétrés (aucune injection) et acceptés tels quels. | Aucun (page unique côté hub) |
| 2026-09-18 | 4.5 | D-45-29 — Revue ★ : le tiroir bi-mode relisait `entityKey`/`recordId` à chaque `load` ; sur « Mes approbations », un « Réessayer » ou un rechargement 409 après perte de la sélection basculait vers la route de conception ⇒ 403 + toast global pour un lecteur. | Corrigé : portée `{ entityKey, recordId }` figée à l'ouverture (`scope` signal), utilisée par `load()` et pour masquer « Ouvrir l'origine » ; +1 `it`. `studio.routes.spec.ts` renforcé : liste de permissions non vide et exactement 4 routes ouvertes au lecteur seul. | Tiroir : même route du début à la fin |
| 2026-09-18 | 4.6 | D-46-01 — Le panneau « instances récentes » du concepteur plafonnait à 20 sans l'afficher ; la route `GET workflows/{id}/instances` n'est pas paginée (borne 1..200, défaut 50). | Appliqué (4.6a2) : `max=50` + invite « Les 50 instances les plus récentes sont affichées. » ; **pagination complète reportée à v1.1** « Historique » (R17), aucune fausse pagination client. | Panneau : 50 instances + invite |
| 2026-09-18 | 4.6 | D-46-02 — Hub « Toutes les tables » : page unique de 200 + message de troncature (résiduel de 4.5f). | Appliqué (4.6a1) : pagination **serveur** `page`/`pageSize=50` + `p-paginator` dès que `totalCount > 50` ; compteur = total serveur ; ordre du serveur conservé (pas de re-tri par page) ; mode `?entity=` inchangé (liste intégrale, filtre local). | Hub paginé, plus de troncature |
| 2026-09-18 | 4.6 | D-46-03 — Le paramètre `search` de `GET workflows` (4.5c) n'était pas branché à l'UI (filtre local sur la page unique). | Appliqué (4.6a1) : saisie anti-rebond 300 ms ⇒ `search` serveur + retour page 1 ; champ vidé ⇒ `search` absent ; en mode table, recherche **locale** inchangée. Jokers `LIKE` acceptés tels quels (D-45-28, documenté). | Recherche sur tout le catalogue |
| 2026-09-18 | 4.6 | D-46-04 — `startedByName` n'était servi que sur l'inbox (D-45-03) ; l'onglet Workflows et le tiroir n'avaient que le guid. | Appliqué (4.6b1) : `WorkflowInstanceDto.startedByName` (optionnel, en fin) résolu en **un lot** par liste (`IStudioUserNameResolver`) sur les 4 routes de lecture (conception + fiche) ; `null` sur les réponses d'écriture (cancel/remind/start) ; portée lecteur : nom servi, `startedBy.email` toujours masqué. | Nom du lanceur partout en lecture |
| 2026-09-18 | 4.6 | D-46-05 — Résiduel D-45-27 : `Steps[].Result` (sorties tronquées) et `Steps[].Error` restaient servis au lecteur ; revue ★ 4.6 : l'`Error` au niveau instance aussi (détail ET liste de la fiche). | Appliqué (4.6b2 + retours ★ 4.6i) : portée lecteur ⇒ `Result`/`Error` nullés sur toutes les étapes ET `Instance.Error` nullé sur les deux routes de la fiche (surface strictement minimale ; le tiroir ne les rend pas) ; route de conception intacte (étape et instance en échec : `Error` servi, verrouillé par un test symétrique). | Résiduel D-45-27 levé |
| 2026-09-18 | 4.6 | D-46-06 — UI « Demandé par » pour les instances (D-45-03 levé côté serveur en 4.6b1). | Appliqué (4.6c1) : colonne « Demandé par » dans l'onglet Workflows de la fiche (`startedByName ?? '—'`) ; tiroir : nom du lanceur, guid si non servi, « Système » si automatique ; mocks Playwright alignés. | Parité inbox / fiche / tiroir |
| 2026-09-18 | 4.6 | D-46-07 — D-44-95 : toasts muets sur le hub et le concepteur (aucun hôte `<p-toast>`). | Appliqué (4.6d1) : `<p-toast styleClass="studio-theme" />` dans le shell de chaque page (motif de la page approbations) ; unicité préservée (pages propres, jamais embarquées ; l'onglet garde l'hôte de la fiche — D-44-89). | Succès/erreurs des écritures visibles |
| 2026-09-18 | 4.6 | D-46-08 — D-44-96 : annulation en un clic sur l'onglet + boutons icône sans `aria-label`, `p-select` sans nom, `<th>` vide. | Appliqué (4.6d2) : confirmation **inline** avec motif optionnel borné à 500 (saisie ET envoi), même motif que le tiroir (D-44-26, pas de `ConfirmationService.prompt`) ; panneau refermé au succès, ouvert en erreur ; `aria-label` sur les 3 boutons et le sélecteur, en-tête d'actions en `sr-only`. | Annulation confirmée + a11y |
| 2026-09-18 | 4.6 | D-46-09 — La clé de jonction proposée utilisait le préfixe de repli `v_` (réservé aux vues) ; la convention serveur est `{a}_{b}` sans préfixe. | Appliqué (4.6e) : `slugifyKey({source}_{cible})` sans préfixe ; les jonctions `v_*` existantes restent valides (aucune migration). En pratique le repli ne s'appliquait pas (les clés d'entités commencent par une lettre) : hygiène. | Clé alignée sur le serveur |
| 2026-09-18 | 4.6 | D-46-10 — Ouvrir `StudioAiCapabilitiesController` aux lecteurs (`custom_records:read`) pour fiabiliser l'accès à « Mes approbations » ? | **Reporté, motivé** : le payload est une configuration IA de conception (annonces de drapeaux, libellés de modèles) ; l'accès lecteur est déjà résolu par la sonde du badge (4.5d1, D-45-12) ; élargir la route n'ajoute aucune fonctionnalité et élargit la surface. | Aucun |
| 2026-09-18 | 4.6 | D-46-T01 — D-44-91 : 52 `acceptButtonStyleClass: 'p-button-*'` sans effet sur les boutons Bootstrap du wrapper. | Appliqué (4.6T1) : (A) `normalizeConfirmButtonClass` pure et exportée dans `ConfirmationService` (table `p-button-* → btn-*`, inconnu tel quel), appliquée à `confirm` et `prompt` ; (B) réécriture mécanique des 52 appelants vers `btn-danger` (35), `btn-success` (13), `btn-secondary` (2), `btn-primary` (2). La normalisation reste en place pour les futurs appelants. | Boutons de confirmation colorés comme prévu |
| 2026-09-18 | 4.6 | D-46-T02 — D-44-56 : requêtes média de largeur dispersées (`matchMedia` ×2, `BreakpointObserver` ×1 dans le Studio). | Appliqué (4.6T2) : `core/services/viewport.service.ts` (`BreakpointObserver` ⇒ signaux `isWide` 1 280 px / `isNarrow` 1 279 px / `prefersReducedMotion`, + `matches(query)` ; injecteur capturé pour un appel hors contexte) ; migration des 3 usages Studio (page approbations, calendrier — seuil 767 px conservé —, concepteur). Seuils strictement inchangés. **Laissés, consignés** : `prefers-reduced-motion` marketing/3D (×10) et lectures ponctuelles `innerWidth` (×6) — sémantique différente (mouvement réduit ≠ viewport ; lecture au clic ≠ réactivité). | Une source pour la largeur du Studio |
| 2026-09-18 | 4.7 | D-47-20 — Aperçu d'une vue non enregistrée (R3) : nouvel endpoint `POST records/{entityKey}/views/preview` (policy `StudioDesignForms`) exécutant la définition du corps via `RecordViewRunExecutor` (dispatch par mode factorisé, partagé 1:1 avec `run`) — **sans persistance, sans audit, sans quota** ; `PreviewRecordViewRequest` sans `search` ni `extraFilters`. | Appliqué (4.7v1) : 6 faits Infra + 1 contrat API ; registre R3 clos. | Aucun (route additive) |
| 2026-09-18 | 4.7 | D-47-21 — Pas de section « concepteur de vues » dans le guide utilisateur : la rubrique « 7.3 Les vues » n'existe pas (13-studio-ia.md ne couvrait pas les vues hors proposition IA). | Section « Les vues enregistrées » créée dans `docs/utilisateur/13-studio-ia.md` (sélecteur, kanban/calendrier, concepteur + aperçu du brouillon). | Doc utilisateur complétée |
| 2026-09-18 | 4.7 | D-47-22 — L'aperçu du concepteur exécutait la version **persistée** et se masquait au changement de mode (`previewModeChanged`). | Appliqué (4.7v2) : runner `preview` (POST `views/preview` avec le brouillon, sans id), anti-rebond 300 ms piloté par effet sur `definition()` (première émission ignorée, peuplement initial ignoré via `applyView`), `previewEligible` = `canSave` moins nom/clé, masque de mode supprimé, erreur réseau inline sans toast. | Aperçu = brouillon, création comprise |
| 2026-09-18 | 4.7 | D-47-23 — En lecture seule, l'aperçu n'exécute plus la version enregistrée (le hint « réservé aux concepteurs » le remplace) : régression assumée du panneau, les lecteurs gardent le `run` via la page Données. | Accepté : l'aperçu en direct est une fonction de conception ; surface lecteur inchangée ailleurs. | Hint `previewDesignOnly` côté lecteur |
| 2026-09-18 | 4.7 | D-47-40 — Attribut de liaison sur la jonction N-N (R4) : `CreateManyToManyRelationRequest.junctionAttributeLabel?` (ajouté **en fin** de DTO) crée un 3ᵉ champ `Number` (non requis, non unique) après les deux champs de liaison, clé = `StudioKey.Slugify(label)` ; garde **avant toute écriture** (vide, forme invalide, clé réservée, collision avec une clé de liaison ⇒ `Validation.junctionAttributeLabel`) ; `ManyToManyRelationDto.attributeField?` en fin ; audit `junctionAttributeKey`. | Appliqué (4.7r1) : 5 faits Infra + 1 unicité (la paire reste contrôlée avec un attribut) + 1 contrat API ; registre R4 clos. | `FieldCount` 2 → 3 quand l'attribut est demandé |
| 2026-09-18 | 4.7 | D-47-41 — Dialogue N-N : le champ « Attribut de liaison » (désactivé « Bientôt » en v1) devient actif et envoie `junctionAttributeLabel` (vide ⇒ `null`) ; libellé `junctionAttributeSoon` retiré du catalogue. **Correctif pré-existant consigné** : le test e2e « fiche Liés 409 » échouait déjà sur `-v3`, `-c2` et `c28cc35d` (bissection) — la sonde `GET records/{key}/{id}/workflow-instances` tombait sur le filet `**/api/**` (objet paginé ⇒ `TypeError … .filter is not a function` en boucle) ; les mocks e2e enregistrent désormais une route `workflow-instances ⇒ ok([])` après `records/${key}/*` (ordre inverse Playwright). | Appliqué (4.7r2) : Karma relations + service 38/38, Playwright m2m 4/4 ; incident hors périmètre v1.1 corrigé au passage. | Dialogue conforme aux maquettes 21–22 |
| 2026-09-18 | 4.7 | D-47-42 — Onglet « Liés » : quantité (attribut de liaison) affichée par lien, saisie à l'ajout, édition inline via `PATCH records/{jonction}/{id}` (rowVersion ; 409 ⇒ « Modifié entre-temps — liste rechargée. »). **Arbitrage n° 4** : l'attribut est résolu en lisant `GET records/{jonction}/schema` (premier champ actif non-relation par `SortOrder`, cache `shareReplay(1)`, 404 ⇒ dégradé silencieux) plutôt qu'en étendant `EntityRelationDto` — aucun nouveau champ serveur sur la projection relations. | Appliqué (4.7r3) : Karma relations 39/39 (+8), Playwright m2m 4/4. | Aucun endpoint dédié « linked » créé |
| 2026-09-18 | 4.7 | D-47-43 — **Arbitrage n° 5** : le `PATCH` d'un enregistrement de jonction reste gardé par le drapeau **`EnableStudioRecordViews`** (introduit en 4.6 pour le PATCH générique) et non par `manyToManyEnabled` ; si le drapeau est coupé, l'appel répond 404 et le frontend signale l'échec **en ligne** (l'édition n'est pas masquée à l'avance). | Accepté : cohérent avec l'existant (un seul drapeau couvre le PATCH de données) ; consigné dans `studio-many-to-many.md` §5bis. | Édition indisponible si le drapeau vues est coupé |
| 2026-09-18 | 4.7 | D-47-44 — Puces N-N inline dans la fiche (captures 21–22) : nouveau `studio-link-chips-editor` monté sous la carte du formulaire, une carte par relation N-N. **Arbitrage n° 6** : puces en **édition seulement** (`recordId` requis, même garde que l'onglet « Liés ») et `DynamicFormComponent` **non modifié** — une relation N-N n'est pas un champ du formulaire. Erreurs 409 (doublon, périmé) en ligne dans la carte ; toast de succès via l'hôte de la fiche. | Appliqué (4.7r4) : Karma puces +8 / fiche +2, suite Studio 745/745, Playwright m2m 5/5. | Création d'enregistrement inchangée (pas de puces) |
| 2026-09-18 | 4.7 | D-47-45 — **Arbitrage n° 8** : l'attribut de liaison n'est pas exposé au spec IA (`relations[]` de `studio_plan_system`/`studio_generate_system` inchangé) — la génération d'un attribut de jonction par l'assistant est reportée (le champ se saisit au dialogue après génération). | Accepté : borne de scope v1.1 ; consigné dans `studio-many-to-many.md` §8.1. | Aucun |
