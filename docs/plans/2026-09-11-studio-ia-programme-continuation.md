...
| 2026-09-18 | 4.6 | D-46-05 — Résiduel D-45-27 : `Steps[].Result` (sorties tronquées) et `Steps[].Error` restaient servis au lecteur. | Appliqué (4.6b2) : portée lecteur ⇒ `Result`/`Error` nullés sur toutes les étapes (surface strictement minimale ; le tiroir ne les rend pas) ; route de conception intacte (étape en échec : `Error` servi). | Résiduel D-45-27 levé |
