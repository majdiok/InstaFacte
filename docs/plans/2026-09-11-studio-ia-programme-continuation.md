...
| — | — | — | — | — |
| 2026-09-12 | 1.4 | Thème D2 : 171 alias `--p-*` / `--color-*` redéfinis dans `.studio-theme` (le plan en estimait ~122) ; `.dark-mode .studio-theme` étendu aux surfaces et bordures ; overlays `appendTo="body"` traités aussi dans `studio-view-designer` et `studio-entity-designer` (6 designers au total, pas 4). | Conservé : le bloc reste ≤ 8 Ko dans `styles.css`, aucun alias global touché. | Aucun hors `/studio/**` ; `/dashboard` reste bleu (smoke 91). |
| 2026-09-12 | 1.4 | A19 : le plan prévoyait `cancelPlan()` puis `send()` enchaînés depuis la page ; en pratique le `send` partait avant la fin de l'annulation (course) et le store pouvait recevoir le nouveau `studio_plan` avant la réponse `cancel`. | Nouvelle méthode `StudioAiSessionStore.abandonPlanAndSend(text, options)` : `cancel` séquencé, puis `send` ; la page n'orchestre plus. | Contrat backend inchangé (`POST plans/{id}/cancel` puis `POST ai/chat/stream`). |
| 2026-09-12 | 1.4 | R21 : le plan décrivait le bandeau doublons comme informatif avec deux boutons « à brancher ». | Boutons opérationnels dès 1.4 : `reuseExistingTable` (`existingKey` + `PUT spec`), `renameDuplicate` (suffixe « (2) » sur clé/singulier/pluriel + surbrillance) ; `dismissedDuplicateRefs` évite la réapparition d'un doublon traité ou fermé. | `summary.duplicates[]` consommé tel que livré par la PR 1.3. |
| 2026-09-12 | 1.4 | `resetConversation()` : le plan la décrivait sans retour ; le toast « N plan(s) annulé(s) » exige le compteur renvoyé par `cancel-pending`. | Signature `resetConversation(done?: (count) => void)` ; `cancel-pending` est **toujours** appelé (plans d'autres sessions/onglets), `DELETE conversations/{id}` seulement si une conversation existe. | Aucun. |
| 2026-09-12 | 1.4 | Pages « Mes projets » et « Bibliothèque de modèles » : le plan les plaçait dans `studio/ai/*.component.ts` à plat. | Sous-dossiers `studio/ai/projects/` et `studio/ai/templates/` (spec à côté) ; routes `ai/projects` / `ai/templates` inchangées. | Aucun. |
| 2026-09-12 | 1.4 | Composeur pendant un plan : la maquette n'en montre qu'un (compose) ; sans second composeur, A19 était inatteignable une fois l'aperçu affiché (le composeur était désactivé sur `hasPlan()`). | Second composeur `.studio-ai-page__followup` sous l'aperçu ; `composerDisabled` ne dépend plus que de `busy()`. | Aucun. |
| 2026-09-12 | 1.4 | Actions Reprendre / Ouvrir le système / Utiliser ce modèle / Retour à l'atelier prévues en `<button (click)=router.navigate>`. | Liens réels `<a pButton [routerLink] [queryParams]>` (href, ouverture dans un nouvel onglet, accessibilité). | Aucun. |
| 2026-09-12 | 1.4 | Carte d'intention **Workflow** : la maquette la montre active ; le backend workflow (PR 4.x) n'existe pas encore. | Activée uniquement si `capabilities.workflowToolsEnabled = true` (prompt `workflowPrompt`), sinon « Bientôt » comme **Page**. | Dépend du champ `workflowToolsEnabled` livré par la PR 2.1. |
| 2026-09-12 | 1.4 | Paramètres d'URL `?intent=`, `?template=`, `?plan=` : le plan ne précisait pas leur cycle de vie. | Consommés une seule fois puis retirés de l'URL (`replaceUrl`) pour éviter la réouverture au rechargement ; `?plan=` d'un plan non `Pending` ⇒ toast `errors.planNotPending`. | Aucun. |
| 2026-09-12 | 1.4 | Smoke tests QA numérotés **91–94** (registre §9.4) ; chapitre utilisateur `13-studio-ia.md` (le plan disait « 13-studio-ia » sans préciser l'icône) avec icône `fa-wand-magic-sparkles` (Font Awesome 7 déjà chargé). | Conservé. | Aucun. |
