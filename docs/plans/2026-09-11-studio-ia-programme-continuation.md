...
| 2026-09-13 | 2.2 | `SystemPromptCacheRevision` : le plan v1 §4.3 n'indiquait pas explicitement s'il fallait l'incrémenter pour la nouvelle règle 3e (conditionnée par un drapeau déjà présent dans la clé de cache tenant/drapeaux). | Laissé à `"v5"` (pas d'incrément) : le cache du prompt StudioBuilder dépend déjà du tenant et du jeu de drapeaux effectif, `EnableStudioManyToMany` en fait implicitement partie via la clé de cache existante. | aucun |
