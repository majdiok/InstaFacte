...
| 2026-09-18 | 4.7 | D-47-43 — **Arbitrage n° 5** : le `PATCH` d'un enregistrement de jonction reste gardé par le drapeau **`EnableStudioRecordViews`** (introduit en 4.6 pour le PATCH générique) et non par `manyToManyEnabled` ; le frontend masque l'édition de la quantité si l'appel répond 404. | Accepté : cohérent avec l'existant (un seul drapeau couvre le PATCH de données) ; consigné dans `studio-many-to-many.md` §5bis. | Édition indisponible si le drapeau vues est coupé |
