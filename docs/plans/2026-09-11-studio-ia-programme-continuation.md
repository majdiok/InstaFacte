...
- **D-26** (revue 4.2d, appliquée en 4.2e) : le `StudioWorkflowResumeJob` isole désormais chaque expiration d'approbation — `try/catch` (concurrence + générique) par itération avec warning, comme le reaper et les reprises : une course sur le `RowVersion` ne saute plus les phases reprises/purge du tenant pour le tick. `StudioWorkflowResumeJob` traite explicitement `StudioWorkflowRunOutcome.Skipped` et logue un warning si la sérialisation du contexte échoue (statut `expired` perdu). `StudioWorkflowRunner` : titre de repli `StarterUnavailable` uniformisé en « Workflow supprimé en échec » (sans guillemets, cohérent avec le chemin nominal).
- **D-27** (revue 4.2e, appliquée en 4.2f) : `ListMyApprovalsQueryHandler` met en cache définitions, entités et clés de libellé par identifiant (N+1 borné : seules instances et enregistrements restent par élément, plafond 200) ; la non-atomicité décision/instance est documentée comme contrat (le job `studio-workflow-resume` réconcilie via `ListDueAsync`, nouvelle tentative ⇒ 409) ; `ExtractDisplay` intercepte `Exception` (nœud non-objet) ; le repli sur l'ancien `ContextJson` si `Serialize()` échoue est documenté (le moteur ignore alors le marqueur de décision au prochain tick — cas extrême accepté).
