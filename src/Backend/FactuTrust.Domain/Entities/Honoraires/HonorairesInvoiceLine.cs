...
public sealed class HonorairesInvoiceLine : Entity
{
    ...
    private HonorairesInvoiceLine() { }

    internal static Result<HonorairesInvoiceLine> Create(
        ...
        string? activityCode = null)
    {
        if (string.IsNullOrWhiteSpace(designation))
            return Result.Failure<HonorairesInvoiceLine>(Error.Validation("Designation", "La désignation est obligatoire"));
        if (quantity <= 0)
            return Result.Failure<HonorairesInvoiceLine>(Error.Validation("Quantity", "La quantité doit être supérieure à zéro"));
        if (discountPercent is < 0 or > 100)
            return Result.Failure<HonorairesInvoiceLine>(Error.Validation("DiscountPercent", "La remise doit être comprise entre 0% et 100%"));

        var line = new HonorairesInvoiceLine
        {
