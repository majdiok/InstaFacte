...
public sealed class ExchangeService : IExchangeService
{
    ...
    public async Task<IReadOnlyList<ExchangeThreadListItemDto>> ListThreadsAsync(
        Guid homeTenantId,
        TenantKind tenantKind,
        FirmDossierAccessScope? firmScope,
        CancellationToken cancellationToken = default)
    {
        var query = _db.ExchangeThreads.AsNoTracking()
            .Join(_db.FirmClientAssignments.AsNoTracking(),
        ...
        if (tenantKind == TenantKind.AccountingFirm)
        {
            ...
            {
                var accessible = await _dossierAccess.GetAccessibleCompanyTenantIdsAsync(homeTenantId, scope, cancellationToken);
        ...
        var names = await GetTenantNamesAsync(companyIds.Concat(firmIds).Distinct().ToList(), cancellationToken);
        var firmProfiles = await _db.AccountingFirmProfiles.AsNoTracking()
        ...
        return rows.Select(t =>
        {
            ...
            return new ExchangeThreadListItemDto(
                ...
                counterpart, t.Status, t.LastActivityAt, 0, t.Subject);
        }).ToList();
    }

    public async Task<Result<ExchangeThreadDetailDto>> GetThreadAsync(
        ...
        FirmDossierAccessScope? firmScope, CancellationToken cancellationToken = default)
    {
        ...
        return Result.Success(await MapDetailAsync(access.Value, cancellationToken));
    }
    ...
    public async Task<Result<IReadOnlyList<ExchangeMessageDto>>> GetMessagesAsync(
        Guid threadId, Guid homeTenantId, TenantKind tenantKind, Guid userId, string? userRole,
        FirmDossierAccessScope? firmScope, DateTime? after, CancellationToken cancellationToken = default)
    {
        var access = await ResolveAccessibleThreadAsync(threadId, homeTenantId, tenantKind, userRole, firmScope, cancellationToken);
        if (access.IsFailure)
            return Result.Failure<IReadOnlyList<ExchangeMessageDto>>(access.Error);

        var isFirm = tenantKind == TenantKind.AccountingFirm;
        var q = _db.ExchangeMessages.AsNoTracking().Where(m => m.ThreadId == threadId);
        if (!isFirm)
            q = q.Where(m => m.Visibility == ExchangeMessageVisibility.ClientVisible);
        if (after.HasValue)
            q = q.Where(m => m.SentAt > after.Value);

        var messages = await q.OrderBy(m => m.SentAt).ToListAsync(cancellationToken);
        return Result.Success<IReadOnlyList<ExchangeMessageDto>>(await MapMessagesAsync(messages, cancellationToken));
    }
    ...
    public async Task<Result<IReadOnlyList<ExchangeRequestDto>>> ListRequestsAsync(
        Guid threadId, Guid homeTenantId, TenantKind tenantKind, Guid userId, string? userRole,
        FirmDossierAccessScope? firmScope, CancellationToken cancellationToken = default)
    {
        var access = await ResolveAccessibleThreadAsync(threadId, homeTenantId, tenantKind, userRole, firmScope, cancellationToken);
        ...
        var rows = await _db.ExchangeRequests.AsNoTracking()
    ...
    public async Task<Result<IReadOnlyList<ExchangeTaskDto>>> ListTasksAsync(
        Guid threadId, Guid homeTenantId, TenantKind tenantKind, Guid userId, string? userRole,
        FirmDossierAccessScope? firmScope, CancellationToken cancellationToken = default)
    {
        var access = await ResolveAccessibleThreadAsync(threadId, homeTenantId, tenantKind, userRole, firmScope, cancellationToken);
        ...
        var rows = await _db.ExchangeTasks.AsNoTracking()
    ...
    public async Task<Result<IReadOnlyList<ExchangeDocumentDto>>> ListDocumentsAsync(
        Guid threadId, Guid homeTenantId, TenantKind tenantKind, Guid userId, string? userRole,
        FirmDossierAccessScope? firmScope, CancellationToken cancellationToken = default)
    {
        var access = await ResolveAccessibleThreadAsync(threadId, homeTenantId, tenantKind, userRole, firmScope, cancellationToken);
        ...
        var rows = await _db.ExchangeDocuments.AsNoTracking()
    ...
    public async Task<Result<IReadOnlyList<ExchangeAuditEventDto>>> GetHistoryAsync(
        Guid threadId, Guid homeTenantId, TenantKind tenantKind, Guid userId, string? userRole,
        FirmDossierAccessScope? firmScope, int page, int pageSize, CancellationToken cancellationToken = default)
    {
        ...
        var rows = await _db.ExchangeAuditEvents.AsNoTracking()
    ...
    public async Task<ExchangeUnreadSummaryDto> GetUnreadSummaryAsync(
        ...
        FirmDossierAccessScope? firmScope, CancellationToken cancellationToken = default)
    {
        var threads = await ListThreadsAsync(homeTenantId, tenantKind, firmScope, cancellationToken);
        if (threads.Count == 0)
        ...
        var messages = await _db.ExchangeMessages.AsNoTracking()
            .Where(m => threadIds.Contains(m.ThreadId) && m.AuthorUserId != userId)
            .Where(m => tenantKind == TenantKind.AccountingFirm
                || m.Visibility == ExchangeMessageVisibility.ClientVisible)
            .Select(m => new { m.Id, m.ThreadId })
            .ToListAsync(cancellationToken);

        var messageIds = messages.Select(m => m.Id).ToList();
        var readIds = await _db.ExchangeMessageReads.AsNoTracking()
            .Where(r => r.UserId == userId && messageIds.Contains(r.MessageId))
            .Select(r => r.MessageId)
            .ToListAsync(cancellationToken);
        var readSet = readIds.ToHashSet();

        var perThread = messages
            .Where(m => !readSet.Contains(m.Id))
            .GroupBy(m => m.ThreadId)
            .Select(g => new ExchangeThreadUnreadDto(g.Key, g.Count()))
            .ToList();
    ...
    private async Task<Result<ExchangeThread>> ResolveAccessibleThreadAsync(
    ...
    {
        var thread = await _db.ExchangeThreads.AsNoTracking()
        ...
        return await EnsureAccessAsync(thread, homeTenantId, tenantKind, userRole, firmScope, cancellationToken);
    }
        ...
        FirmDossierAccessScope? firmScope, CancellationToken cancellationToken)
    {
        ...
        var access = await EnsureAccessAsync(thread, homeTenantId, tenantKind, userRole, firmScope, cancellationToken);
        return access.IsFailure ? Result.Failure<ExchangeThread>(access.Error) : Result.Success(thread);
    }

    private async Task<Result<ExchangeThread>> EnsureAccessAsync(
        ExchangeThread thread, Guid homeTenantId, TenantKind tenantKind, string? userRole,
        FirmDossierAccessScope? firmScope, CancellationToken cancellationToken)
    {
        var assignmentActive = await _db.FirmClientAssignments.AsNoTracking()
        ...
        if (firmScope is { } scope &&
            !await _dossierAccess.CanAccessAssignmentAsync(homeTenantId, scope, thread.FirmClientAssignmentId, cancellationToken))
            return Result.Failure<ExchangeThread>(Error.Forbidden(FirmDossierAccessService.NotAssignedMessage));

        return Result.Success(thread);
    }

    private async Task<ExchangeThreadDetailDto> MapDetailAsync(ExchangeThread thread, CancellationToken cancellationToken)
    {
        var names = await GetTenantNamesAsync(new[] { thread.CompanyTenantId, thread.FirmTenantId }, cancellationToken);
        var firmProfile = await _db.AccountingFirmProfiles.AsNoTracking()
            .Where(p => p.TenantId == thread.FirmTenantId)
            .Select(p => p.DisplayName)
            .FirstOrDefaultAsync(cancellationToken);

        var companyUsers = await _userManager.Users.AsNoTracking()
            .Where(u => u.TenantId == thread.CompanyTenantId && u.IsActive)
            .OrderBy(u => u.LastName)
            .Take(20)
            .ToListAsync(cancellationToken);
        var firmUsers = await _userManager.Users.AsNoTracking()
        ...
        foreach (var u in companyUsers)
        {
            ...
            if (!roles.Contains(nameof(UserRole.Administrator)))
                continue;
    ...
    private async Task<IReadOnlyList<ExchangeMessageDto>> MapMessagesAsync(
        IReadOnlyList<ExchangeMessage> messages, CancellationToken cancellationToken)
    {
        ...
        var reads = await _db.ExchangeMessageReads.AsNoTracking()
            .Where(r => ids.Contains(r.MessageId))
            .ToListAsync(cancellationToken);
        var docs = await _db.ExchangeDocuments.AsNoTracking()
            .Where(d => d.MessageId != null && ids.Contains(d.MessageId.Value))
            .ToListAsync(cancellationToken);

        var readerIds = reads.Select(r => r.UserId).Distinct().ToList();
        var readerNames = await _userManager.Users.AsNoTracking()
    ...
    private async Task<Dictionary<Guid, string>> GetTenantNamesAsync(IReadOnlyList<Guid> ids, CancellationToken cancellationToken)
    {
        if (ids.Count == 0)
