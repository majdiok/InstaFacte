...
public static class NctStatementBuilder
{
        ...
        decimal netResult, NctDisposalAggregates disposals)
    {
        ...
        // CFECART = −NetResult(N-1). Result(d) = −Σ net(6/7) = NetResult(year) si non affecté, 0 si
        ...
        // classe1(prev) − NetPnl(prev) = classe1(prev) + NetResult(N-1). No-op quand Result(prev) = 0
        // (résultat affecté/ancré) → pas de régression pour les tenants déjà rapprochés.
        var financing = -dClass1 - d50 - Result(prev);
