...
@Component({
  ...
  template: `
    <div class="me-balance-indicator"
         ...
         [class.me-unbalanced]="!isBalanced()">
      <div class="me-balance-text">
        <span class="me-balance-icon" aria-hidden="true">{{ isBalanced() ? '✓' : '⚠' }}</span>
        @if (isBalanced()) {
          <span class="me-balance-label">Écriture équilibrée</span>
        } @else {
  ...
  `,
  styles: `
    .me-balance-indicator { display:flex; justify-content:space-between; align-items:center; gap:var(--spacing-3); padding:var(--spacing-3) var(--spacing-4); border-radius:var(--radius-md); font-weight:var(--font-weight-semibold); margin:var(--spacing-4) 0; }
