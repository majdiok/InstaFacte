...
function mockAccountingService(overrides: { reversesEntryId?: string | null } = {}): AccountingService {
  return {
    getJournal: () =>
      of({
        success: true,
...
async function setupFixture(
  authOverrides: Parameters<typeof mockAuthService>[0] = {},
  accountingOverrides: Parameters<typeof mockAccountingService>[0] = {}
) {
  ...
  await TestBed.configureTestingModule({
    ...
    providers: [
      ...
      provideRouter([]),
      { provide: AccountingService, useValue: mockAccountingService(accountingOverrides) },
...
describe('JournalComponent', () => {
  ...
  describe('with draft extourne', () => {
    it('renders edit action for delegated firm manager', async () => {
      const fixture = await setupFixture({ isFirmManager: true }, { reversesEntryId: 'entry-original' });
      const el = fixture.nativeElement as HTMLElement;
      const buttons = el.querySelectorAll('app-button');
      let found = false;
      for (const btn of Array.from(buttons)) {
    ...
    it('hides edit action for company (non-firm) user', async () => {
      const fixture = await setupFixture(
        { isAccountingFirm: false, isDelegatedMode: false, isFirmManager: false },
