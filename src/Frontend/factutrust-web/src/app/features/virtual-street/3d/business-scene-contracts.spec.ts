...
function glbAsset(assetKey: string, path: string): Record<string, unknown> {
  return {
    assetKey,
    kind: 'glb',
    path,
...
function variant(modelAssetKey: string): Record<string, unknown> {
  return {
    modelAssetKey,
...
function scene(sceneKey: string, kind: string, modelPrefix: string): Record<string, unknown> {
  return {
    sceneKey,
    kind,
    units: 'meters',
